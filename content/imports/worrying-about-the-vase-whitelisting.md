...
lw-linkpost-url: https://www.lesswrong.com/posts/H7KB44oKoSjSCkpzL/worrying-about-the-vase-whitelisting
...
_Rare LEAKED footage of Mickey trying to catch up on his alignment theory after instantiating an unfriendly genie \[colorized, 2050\]._$^2$
...
*   If an agent _did_ somehow come up with plans that involved blowing up distant stars, that would indeed constitute [astronomical waste](https://nickbostrom.com/astronomical/waste.html).$^\text{a triple pun?}$ Whitelisting doesn't _solve_ the problem of assigning too much weight to events outside our corner of the neighborhood, but it's an improvement.
...
*   Furthermore, there is no need to identify objects on multiple levels (_e.g._, a forest, a tree in the forest, and that tree's bark need not all be identified concurrently).
...
*   _Latent space whitelisting:_ the learned latent space encapsulates most of the relevant side effects. This is a slightly weaker assumption.
...
_Edit: [a potential solution](/overcoming-clinginess-in-impact-measures) to clinginess, with its own drawbacks._
